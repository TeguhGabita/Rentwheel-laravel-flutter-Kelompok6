    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <form method="GET" class="w-full flex flex-col sm:flex-row gap-3">
            ...
            <select name="kategori_id" onchange="this.form.submit()"
                    class="w-full sm:w-44 px-3 py-2.5 text-sm rounded-xl border border-slate-200 focus:border-amber-400 focus:ring-amber-400 focus:outline-none focus:ring-1">
            ...
            <select name="status" onchange="this.form.submit()"
                    class="w-full sm:w-40 px-3 py-2.5 text-sm rounded-xl border border-slate-200 focus:border-amber-400 focus:ring-amber-400 focus:outline-none focus:ring-1">
            ...
            @if (request('search') || request('kategori_id') || request('status'))
                <a href="{{ route('admin.mobil.index') }}"
                   class="inline-flex items-center justify-center px-3 py-2.5 text-sm font-medium text-slate-500 hover:text-slate-700 whitespace-nowrap">
                    Reset
                </a>
            @endif
        </form>
        ...
        <a href="{{ route('admin.mobil.create') }}"
           class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-semibold hover:bg-slate-800 transition-colors whitespace-nowrap flex-shrink-0">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-lg bg-slate-100 flex items-center justify-center overflow-hidden flex-shrink-0">
                                        @php
                                            // Cek apakah foto ada di database DAN file-nya benar-benar ada di disk 'public'
                                            $adaFoto = !empty($mobil->foto) && \Illuminate\Support\Facades\Storage::disk('public')->exists($mobil->foto);
                                        @endphp
                                        ...
                                        @if ($adaFoto)
                                            <img
                                                ...
                                                onerror="this.onerror=null; this.src='https://placehold.co/80x80?text=Mobil';"
                                            >
                                        @else
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-slate-500">
                                Belum ada data mobil{{ (request('search') || request('kategori_id') || request('status')) ? ' yang cocok dengan filter.' : '.' }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
